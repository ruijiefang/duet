...
(** Given a [binding] whose support contains the variables of [implicant],
    and a valuation [m] whose extension to integer and fractional variables
    satisfies [implicant],
    [local_project srk binding m elim implicant = (P, L)] where
    - [P] and [L] are only in original dimensions;
    - [m |= formula_of(P) /\ formula_of(L)];
    - For any non-strict inequality [ax >= 0] where [x] is in original
      variables excluding [elim], if [implicant |= ax >= 0],
      then [formula_of(P) |= ax >= 0].
    - (TODO) For any LIRA formula [phi] in original variables excluding [elim],
      if [implicant |= phi], [formula_of(P) /\ formula_of(L) |= phi].

    The entailments [implicant |= ax >= 0] and [implicant |= phi] are
    modulo LIRA and equalities [x] = [x_int] + [x_frac],
    integrality constraints for [x_int],
    and bound constraints for [x_frac], for each variable x that occurs in
    [binding]).
 *)
val local_project:
  'a Syntax.context -> dimension_binding -> 'a Interpretation.interpretation ->
  eliminate_original:int list -> linear_formula -> DD.closed DD.t * IntLattice.t
